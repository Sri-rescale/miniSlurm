...
#include "src/common/rescale_module.h"
...
int main(int argc, char **argv)
{
	 ...
	 * and run it there
	 */
	/* Only run if Rescale flag is set to 0. */
	if (opt.rescale_flag == 0) {
		if (opt.clusters) {
			if (job_req_list) {
	...
	while (true) {
		static char *msg;
		/* Only run if Rescale flag is set to 0. 
		   We will introduce custom errornu values for Rescale 
		   issues */
		if (opt.rescale_flag == 0){
			if (job_req_list)
			...
			printf("Forking to Rescale Job Submission....\n");
			/*error("Rescale unavailable for SBATCH - No API connection - YET....");*/
			rc = rescale_submit(desc);
			/* TO-DO - Put an else for our Rescale job submission here */
			exit(error_exit);
		}
	...
	if (!quiet) {
		/* Only run if Rescale is not activated */
		if (opt.rescale_flag == 0){
			if (!sbopt.parsable) {
		...
		/* TO-DO - Put custom messaging here for Rescale job submissions */
	}

	if (sbopt.wait)
		/* TO-DO - Rescale job wait here? */
